  ...
  return (
    ...
    <main className="animate-fade-in" style={{ background: 'var(--bg-secondary)', minHeight: 'calc(100vh - 80px)', display: 'flex', alignItems: 'flex-start', paddingTop: '2rem', paddingBottom: '2rem' }}>
      <div className="container" style={{ width: '100%', maxWidth: '1400px', display: 'flex', flexWrap: 'wrap', gap: '2rem', padding: '0 1.5rem' }}>
        ...
        {/* LEFT VISUAL CONTENT */}
        <div style={{ flex: '1', minWidth: '350px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', paddingTop: '0' }}>
          <h1 className="hero-title" style={{ fontSize: '3rem', marginBottom: '0.5rem', textAlign: 'left', lineHeight: '1.1' }}>
            Book your <span>demo</span>
          </h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', marginBottom: '1.5rem', maxWidth: '500px' }}>
            Select the modules you are interested in, and we will tailor the walk-through to your specific organizational needs.
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {offerings.map(off => (
              ...
              >
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: '1.4', paddingLeft: '2.35rem', marginTop: '0.25rem' }}>
                  {off.desc}
                </div>
              </div>
            ))}
          ...
          <form style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }} onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem' }}>
              <div style={{ flex: 1 }}>
                ...
                <input name="firstName" type="text" required className="form-input" style={{ padding: '0.6rem 1rem' }} />
              </div>
              <div style={{ flex: 1 }}>
                <label className="form-label" style={{ fontSize: '0.9rem', marginBottom: '0.25rem' }}>Last Name</label>
                <input name="lastName" type="text" required className="form-input" style={{ padding: '0.6rem 1rem' }} />
              </div>
            </div>

            <div style={{ display: 'flex', gap: '1.5rem' }}>
              <div style={{ flex: 1 }}>
                <label className="form-label" style={{ fontSize: '0.9rem', marginBottom: '0.25rem' }}>Work Email</label>
                <input name="email" type="email" required className="form-input" style={{ padding: '0.6rem 1rem' }} />
              </div>
              <div style={{ flex: 1 }}>
                <label className="form-label" style={{ fontSize: '0.9rem', marginBottom: '0.25rem' }}>Company Name</label>
                <input name="company" type="text" className="form-input" style={{ padding: '0.6rem 1rem' }} />
              </div>
            </div>

            <div>
              <label className="form-label" style={{ fontSize: '0.95rem', marginBottom: '0.75rem', fontWeight: '600' }}>I'm interested in:</label>
                ...
                {offerings.map(off => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '0.5rem 1rem', borderRadius: '99px', fontSize: '0.9rem', fontWeight: '500',
                      border: selectedInterests.includes(off.id) ? '2px solid var(--primary-green)' : '1px solid var(--border-light)',
                      background: selectedInterests.includes(off.id) ? 'rgba(16, 185, 129, 0.1)' : 'white',
                      color: selectedInterests.includes(off.id) ? 'var(--primary-green-dark)' : 'var(--text-muted)',
                      transition: 'all 0.2s'
                    }}
                  >
                    {selectedInterests.includes(off.id) && <CheckCircle2 size={16} />}
                    {off.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="form-label" style={{ fontSize: '0.9rem', marginBottom: '0.25rem' }}>Any specific requirements?</label>
